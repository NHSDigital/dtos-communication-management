...
def test_create_message_batch_pact(create_message_batch_pact, message_batch_post_body):
    ...
    create_message_batch_pact.given(
        "A message batch is created"
    ).upon_receiving(
        "A request to create a message batch"
    ).with_request(
        "post", endpoint, headers=headers, body=message_batch_post_body
    ).will_respond_with(
        201, body=expected_create_message_batch_response()
    )
