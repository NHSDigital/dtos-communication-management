...
locals {
  ...
  app_settings = merge(
    config.env_vars_static,
    {
      ...
      DATABASE_HOST     = "${module.regions_config[primary_region].names.postgres-sql-server}.postgres.database.azure.com"
      DATABASE_USER     = "commgt_db_user"
      DATABASE_PASSWORD = "@Microsoft.KeyVault(SecretUri=${module.postgresql_flexible_db[primary_region].db_admin_pwd_keyvault_secret})"

      APPLICATION_ID = "@Microsoft.KeyVault(SecretUri=${data.azurerm_key_vault_secret.application_id[primary_region].versionless_id})"
      NOTIFY_API_KEY = "@Microsoft.KeyVault(SecretUri=${data.azurerm_key_vault_secret.notify_api_key[primary_region].versionless_id})"
      OAUTH2_API_KID = "@Microsoft.KeyVault(SecretUri=${data.azurerm_key_vault_secret.oauth2_api_kid[primary_region].versionless_id})"
      OAUTH2_API_KEY = "@Microsoft.KeyVault(SecretUri=${data.azurerm_key_vault_secret.oauth2_api_key[primary_region].versionless_id})"
      PRIVATE_KEY    = "@Microsoft.KeyVault(SecretUri=${data.azurerm_key_vault_key.private_key[primary_region].versionless_id})"
    }
  )
  ...
  rbac_role_assignments = flatten([
    var.key_vault != {} ? [
      for role in local.rbac_roles_key_vault_user : {
        ...
        scope                = module.key_vault[primary_region].key_vault_id
      }
    ] : [],
    [
      for account in keys(var.storage_accounts) : [
        for role in local.rbac_roles_storage : {
          role_definition_name = role
          scope                = module.storage["${account}-${primary_region}"].storage_account_id
        }
      ]
    ]
  ])
